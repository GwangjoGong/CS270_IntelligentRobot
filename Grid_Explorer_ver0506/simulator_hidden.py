...
class myRobot(pygame.Surface):
    ...
    def draw_robot(self):
        ...
        self.fill(red)
        # Center
        pygame.draw.rect(self, navy, pygame.Rect(
            self.w/2 - 1*scale, self.h/2 - 1*scale, 3*scale-1, 3*scale-1))
        # Color sensors
        pygame.draw.rect(self, yellow, pygame.Rect(
            scale, self.h-2*scale, 2*scale, 2*scale))
        pygame.draw.rect(self, yellow, pygame.Rect(
            self.w-3*scale, self.h-2*scale, 2*scale, 2*scale))
        # Design
        pygame.draw.rect(self, white, pygame.Rect(
            self.w/2 - 3*scale, self.h - 8*scale, 7*scale-1, 8*scale))
...
class Simulator():
    def __init__(self):
        ...
        # Create robot
        self.robot = myRobot(self.screen, init_position,
                             init_angle, robot_size, self.map_size)
        self.update()
    ...
    def draw_map(self):
        ...
        pygame.draw.rect(self.screen, white, pygame.Rect(
            border, border, self.map_size[0]-2*border, self.map_size[1]-2*border))

        for x in range(1, self.map_dim[0]):
            pygame.draw.rect(self.screen, gray, pygame.Rect(
                border + x*block + (x-1)*line, border, line, self.v_line))
        for y in range(1, self.map_dim[1]):
            pygame.draw.rect(self.screen, gray, pygame.Rect(
                border, border + y*block + (y-1)*line, self.h_line, line))

        for i, j in self.lake_blocks:
            pygame.draw.rect(self.screen, blue, pygame.Rect(
                border + i*(block+line), border + j*(block+line), block, block))
        for i, j in self.building_blocks:
            pygame.draw.rect(self.screen, brown, pygame.Rect(
                border + i*(block+line) + padding, border + j*(block+line) + padding, building, building))
